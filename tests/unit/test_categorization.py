...
class FlakyAsyncCategorizer:
    ...
    def __init__(self, failures_by_cluster_id: dict[str, int], categories_by_cluster_id: dict[str, str]) -> None:
        self.failures_by_cluster_id = failures_by_cluster_id
        self.categories_by_cluster_id = categories_by_cluster_id
        self.calls: list[tuple[str, str, str]] = []
        self.attempts_by_cluster_id: dict[str, int] = {}
    ...
    async def acategorize_cluster(
        ...
        attempt = self.attempts_by_cluster_id.get(cluster_id, 0) + 1
        self.attempts_by_cluster_id[cluster_id] = attempt
        if attempt <= self.failures_by_cluster_id.get(cluster_id, 0):
            raise TransientCategorizationError(f"retryable failure for {cluster_id}")
        return ClusterCategorizationResult(
            ...
            response_id=f"mock-{cluster_id}-{attempt}",
        )
...
def test_run_cluster_categorization_retries_transient_failures_and_succeeds(tmp_path, monkeypatch):
    ...
    categorizer = FlakyAsyncCategorizer(
        failures_by_cluster_id={"cluster001": 2},
    ...
    assert categorizer.attempts_by_cluster_id == {"cluster001": 3}
    assert (day_path / "architecture" / "c001_b001_001.jpg").exists()
...
def test_run_cluster_categorization_does_not_partially_mutate_day_on_failed_retries(tmp_path, monkeypatch):
    ...
    categorizer = FlakyAsyncCategorizer(
        failures_by_cluster_id={"cluster002": 5},
        categories_by_cluster_id={
            "cluster001": "architecture",
            "cluster002": "other",
        },
